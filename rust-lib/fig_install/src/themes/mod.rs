...
pub async fn clone_or_update() -> Result<()> {
    match download::clone_git_repo_with_reference(THEMES_REPO, themes_directory().unwrap(), None).await {
        Ok(_) => {
            info!("Cloned themes repo");
        },
        Err(err) => {
            error!("Error cloning themes repo: {err}");
            match download::update_git_repo_with_reference(themes_directory().unwrap(), None).await {
