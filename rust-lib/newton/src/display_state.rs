...
impl DisplayState {
    ...
    pub fn draw_symbol(&mut self, symbol: char, x: i32, y: i32, color: Color, background_color: Color) -> &mut Self {
    ...
    pub fn draw_string(
        ...
        for (i, symbol) in string.as_ref().chars().enumerate() {
            self.draw_symbol(symbol, x + i32::try_from(i).unwrap(), y, color, background_color);
        }
    ...
    pub fn draw_rect(
        ...
        for y in y..(y + height).min(self.height()) {
            for x in x..(x + width).min(self.width()) {
                self.draw_symbol(symbol, x, y, color, background_color);
            }
        }
