...
/// NOTE: This had to be copied from Starscream source since
/// public structs (eg. Frame) that don't define initializers are treated
/// as private and their properties are not accessible. - mschrage
import Foundation
...
public class WSFramer: Framer {
    ...
    private func process() -> ProcessEvent {
        ...
        if !isServer && (isMasked > 0 || RSV1 > 0) && opcode != .pong && !needsDecompression {
            return .failed("masked and rsv data is not currently supported")
        }
        ...
        if !isControlFrame && (opcode != .binaryFrame && opcode != .continueFrame &&
            ...
            return .failed("unknown opcode")
        }
        if isControlFrame && isFin == 0 {
            return .failed("control frames can't be fragmented")
        }
        
        var offset = 2
    
        if isControlFrame && payloadLen > 125 {
            return .failed("payload length is longer than allowed for a control frame")
        }
