...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
  ...
  func application(_ application: NSApplication, open urls: [URL]) {
    ...
    // Local URL scheme is of the form - fig://{app}/{path}?{query}
    guard let components = NSURLComponents(url: urls[0], resolvingAgainstBaseURL: true) else {
      return
    }

    DeepLinkHandler.handle(
      appName: components.host,
      path: components.path,
      queries: URLComponents(string: urls[0].absoluteString)?.queryItems
    )
  }
