...
class Restarter {
  ...
  func restart(launchingIfInactive: Bool = true, completion: (() -> Void)? = nil) {
    if let app = NSWorkspace.shared.runningApplications.filter({
      return $0.bundleIdentifier == self.bundleIdentifier
    }).first {
      self.app = app
      self.app?.terminate()
      self.kvo = self.app!.observe(\.isTerminated, options: .new) { (app, terminated) in
        if terminated.newValue == true {
          NSWorkspace.shared.launchApplication(withBundleIdentifier: self.bundleIdentifier,
                                               options: [.default],
