...
extension URL {
  func relativePath(from base: URL) -> String? {
    ...
    var idx = 0
    while idx < destComponents.count && idx < baseComponents.count
            && destComponents[idx] == baseComponents[idx] {
      idx += 1
    }

    // Build relative path:
    var relComponents = Array(repeating: "..", count: baseComponents.count - idx)
