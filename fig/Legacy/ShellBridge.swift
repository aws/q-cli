...
class ShellBridge {
  ...
  // This fixes an issue where focus would bounce to an application in the previous workspace.
  // Essentially this resets previous application anytime the workspace is changed.
  ...
  @objc func setPreviousApplication(notification: NSNotification!) {
    self.previousFrontmostApplication =
      notification!.userInfo![NSWorkspace.applicationUserInfoKey] as? NSRunningApplication
    print("Deactivated:", self.previousFrontmostApplication?.bundleIdentifier ?? "")
  }

  static func privateCGEventCallback(proxy: CGEventTapProxy,
                                     type: CGEventType,
                                     event: CGEvent,
...
extension Timer {
  ...
  @discardableResult
  static func cancellableDelayWithSeconds(_ timeInterval: TimeInterval,
                                          closure: @escaping () -> Void) -> DispatchWorkItem {
