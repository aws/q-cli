...
class Icon {
  ...
  static func fileIcon(for url: URL) -> NSImage? {
    ...
    if let host = url.host, let qs = url.queryDictionary, let type = qs["type"], host == "icon" {
      ...
      return NSWorkspace.shared.icon(forFileType: fileType).resized(to: NSSize(width: width, height: height))?.overlayBadge(color: color, text: badge)

    }

    if let host = url.host,
       let qs = url.queryDictionary,
       let pid = qs["pid"], host == "icon" {
    ...
    guard isFile || isDirectory.boolValue else {
      var type = NSString(string: fullPath).pathExtension
      if String(resource).last == "/" {
        type = NSFileTypeForHFSTypeCode(OSType(kGenericFolderIcon))
      }

      return NSWorkspace.shared.icon(forFileType: type).resized(to: NSSize(width: width, height: height))?.overlayBadge(color: color, text: badge)
    }
...
extension NSImage {
  ...
  func overlayColor(_ color: NSColor?) -> NSImage {
    ...
    let imageRect = NSRect(origin: NSPoint.zero, size: self.size)
  ...
  func overlayBadge(color: NSColor?, text: String?) -> NSImage {
    ...
    if let bitmapRep = NSBitmapImageRep(
      ...
      let rect = NSRect(x: size.width/2, y: 0, width: size.width/2, height: size.height/2)
