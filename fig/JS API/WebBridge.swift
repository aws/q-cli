...
class WebBridge: NSObject {
  ...
  weak var eventDelegate: WebBridgeEventDelegate?
...
extension WebBridge: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
    ...
    if !WebBridge.authorized(webView: message.webView) {
      message.webView?.evaluateJavaScript("console.log(`Attempted to call fig runtime from unauthorized domain`)",
...
extension WebBridge {
  static func authorized(webView: WKWebView?) -> Bool {
    if let webView = webView, let url = webView.url, let scheme = url.scheme {
      ...
      return scheme == "file"
              || url.host == Remote.baseURL.host
              || url.host == Remote.baseURL.host
              || url.host == "desktop.fig.io"
              || url.host ?? "" == "localhost"
    }
    return false
  }
