...
class TerminalUsageObserver {
  static let terminalApplicationLostFocusNotification: NSNotification.Name =
  Notification.Name("terminalApplicationLostKeyNotification")

  var observer: NSKeyValueObservation?
  var start: Date = Date(timeIntervalSinceNow: 0)
  init() {

    self.observer = NSWorkspace.shared.observe(\.frontmostApplication, options: [.old]) { (_, delta) in
      ...
      if let app = delta.oldValue {
        ...
        if Integrations.bundleIsValidTerminal(app?.bundleIdentifier) {
          NotificationCenter.default.post(name: TerminalUsageObserver.terminalApplicationLostFocusNotification,
                                          object: delta)
        }
        self.start = now
      }
