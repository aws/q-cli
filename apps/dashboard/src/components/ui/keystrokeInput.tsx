...
export function Input ({ command, value, invalid, cancel }: { command: string, value: string[], invalid: boolean, cancel: () => void }) {
  ...
  return (
    <div className="flex gap-1">
      ...
      <div
        className={`flex items-stretch gap-1 p-[2px] py-1 ${(!value) && "pl-3"} text-xs bg-black rounded-md`}
      >
        <div
...
export default function KeystrokeGroup({
  ...
  return (
    ...
    <div className="flex gap-2 flex-wrap">
      {keybindings.map((k: string, i: number) => <Keystroke keybinding={k} key={i} />)}
      {inputOpen ? 
        <Input command={command} value={inputValue as string[]} invalid={isInvalid} cancel={cancelKeystroke} />
       : (
        <button
          onClick={openInput}
