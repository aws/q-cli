...
def profile_step(shell: str, id: str):
    timings = []
    last_timing = None
    child = subprocess.Popen(f'{shell} -x /tmp/{id}',
...
def test(shell: str, stage: str, cycles: int):
    ...
    generate = subprocess.run(
        f'cargo run -p cw_cli --release -- init {shell} {stage}', shell=True, stdout=subprocess.PIPE)
    ...
    for (num, run) in enumerate(runs):
        for i in range(len(run)):
            ...
            if run[i][1] != runs[0][i][1]:
                print(
                    f'discarding run {num} because {run[i][1]} != {runs[0][i][1]}', file=sys.stderr)
                bad_runs.append(num)
    ...
    for (i, line) in enumerate(zip(*runs)):
        ...
        code = line[0][1]
        pretty.append((round(avg_timings/1_000_000, 3), code.decode()[1 if shell == 'zsh' else 2:-1].replace(
