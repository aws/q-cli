...
_fig_autosuggest_bind_widget() {
	...
	case $widgets[$widget] in
		...
		# User-defined widget
		user:*)
			_fig_autosuggest_incr_bind_count $widget
		...
		builtin)
			_fig_autosuggest_incr_bind_count $widget
			eval "_fig_autosuggest_orig_${(q)widget}() { zle .${(q)widget} }"
			zle -N $prefix$bind_count-$widget _fig_autosuggest_orig_$widget
			;;
	...
	# `zle self-insert -w`).
	eval "_fig_autosuggest_bound_${bind_count}_${(q)widget}() {
		_fig_autosuggest_widget_$autosuggest_action $prefix$bind_count-${(q)widget} \$@
	}"

	# Create the bound widget
	zle -N -- $widget _fig_autosuggest_bound_${bind_count}_$widget
}

# Map all configured widgets to the right autosuggest widgets
_fig_autosuggest_bind_widgets() {
	emulate -L zsh
	...
	for widget in ${${(f)"$(builtin zle -la)"}:#${(j:|:)~ignore_widgets}}; do
		if [[ -n ${FIG_AUTOSUGGEST_CLEAR_WIDGETS[(r)$widget]} ]]; then
			...
			_fig_autosuggest_bind_widget $widget modify
		fi
	done
}
