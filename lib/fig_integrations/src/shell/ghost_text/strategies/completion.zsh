...
_fig_autosuggest_capture_postcompletion() {
...
_fig_autosuggest_capture_completion_widget() {
	...
	comppostfuncs=(_fig_autosuggest_capture_postcompletion)
...
zle -N autosuggest-capture-completion _fig_autosuggest_capture_completion_widget

_fig_autosuggest_capture_setup() {
...
_fig_autosuggest_capture_completion_sync() {
	_fig_autosuggest_capture_setup

	zle autosuggest-capture-completion
}

_fig_autosuggest_capture_completion_async() {
	_fig_autosuggest_capture_setup
...
_fig_autosuggest_strategy_completion() {
	...
	[[ -n "$FIG_AUTOSUGGEST_COMPLETION_IGNORE" ]] && [[ "$1" == $~FIG_AUTOSUGGEST_COMPLETION_IGNORE ]] && return

	# Zle will be inactive if we are in async mode
	if zle; then
		zpty $FIG_AUTOSUGGEST_COMPLETIONS_PTY_NAME _fig_autosuggest_capture_completion_sync
	else
		zpty $FIG_AUTOSUGGEST_COMPLETIONS_PTY_NAME _fig_autosuggest_capture_completion_async "\$1"
		zpty -w $FIG_AUTOSUGGEST_COMPLETIONS_PTY_NAME $'\t'
	fi

	{
		# The completion result is surrounded by null bytes, so read the
		# content between the first two null bytes.
		zpty -r $FIG_AUTOSUGGEST_COMPLETIONS_PTY_NAME line '*'$'\0''*'$'\0'
		...
		# Destroy the pty
		zpty -d $FIG_AUTOSUGGEST_COMPLETIONS_PTY_NAME
	}
}
